...
import { readableStreamToString, BrexTransactionsError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new BrexTransactionsError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new BrexTransactionsError(e, e.response?.data)
                }
                if (innerError instanceof BrexTransactionsError) {
                    // Got "BrexTransactionsError" from the above try block
                    throw innerError;
                }
